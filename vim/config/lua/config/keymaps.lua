...
-- %% expands to the directory of the current file in command-line mode
vim.cmd("cnoremap %% <C-r>=expand('%:h').'/'<CR>")
-- %f expands to the filename (tail) of the current file in command-line mode
vim.cmd("cnoremap %f <C-r>=expand('%:t')<CR>")
...
wk.add(vim.list_extend({
  ...
  { '<leader>O',      function() fzf.files({ rg_opts = '--no-ignore', fd_opts = '--no-ignore' }) end, desc = 'Find all files (no gitignore)' },
